...
class DBHelper:
    ...
    @staticmethod
    def sum_by_field_value(collection, field_to_match: str, value: Any, field_to_sum: str, operator: str=None):
    ...
    @staticmethod
    def sum_field_by_date(collection, field, timestamp_field_name = 'timestamp'):
        """Returns docs with field grouped by date"""
        pipeline = [
            {'$addFields': {'date': {'$dateToParts': { 'date': f"${timestamp_field_name}"}}}},
            {'$group': {'_id': {'year': '$date.year', 'month': '$date.month', 'day': '$date.day'},
                        'total': {'$sum': f'${field}'}}},
            { '$sort': {'year': 1, 'month': 1, 'day': 1}},
            {'$project': {'date': {'$dateFromParts':  {'year': '$_id.year', 
                                                      'month': '$_id.month',
                                                      'day': '$_id.day',}}, 
                          'total' : 1}
             }
...
class Journal:
    ...
    def update(self, robot: Robot, data: dict) -> None:
        target_collection = self.cut_journal if isinstance(robot, CutRobot) else self.water_journal
        target_collection.insert_one(data)

    def get_required_amount_of_water(self) -> float:
        today = datetime.today().replace(hour=0, minute=0, second=0)
        todays_water_amount = DBHelper.sum_by_field_value(self.water_journal, 'timestamp', today, 'water_amount', '$gte')
    ...
    def get_water_errors(self):
        daily_water_data = DBHelper.sum_field_by_date(self.water_journal, 'water_amount')
        existing_data = [DailyData(day['date'], day['total']) for day in daily_water_data]
        ...
        errors = [day for day in existing_data
    ...
    def get_cut_errors(self):
        ...
        wrong_weeks = [week for week in week_data
    ...
    def print_last_10_records(self, action: str):
        if action not in self.stored_actions:
            raise ValueError(f'Unknown action. Should be one of {", ".join(self.stored_actions)}')
...
class WaterRobot(Robot):
    ...
    @tank_volume.setter
    def tank_volume(self, new_volume):
    ...
    def water_tree(self, journal: Journal):
        ...
        if required_volume:
            if required_volume <= self._water_level:
                ...
                update_data = {'robot_id': self.id_number,
                               'water_amount': required_volume,
                               'timestamp': datetime.now()}
                journal.update(self, update_data)
            else:
                raise NotEnoughWaterError(f'Current water level is {self.water_level}l,\
